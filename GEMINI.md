...
8. **Loop Stride Optimization**: When rendering massive wide Canvas objects (like a 10,000px wide waveform inside a ScrollView), avoid iterating by `1.0` if possible. Striding by `3.0` and adjusting the drawing width proportionally reduces CPU math and `Path` allocation overhead by 66% with negligible visual impact.
9. **Hover Lifecycle Stability (The `HoverTrackingView` pattern)**: Never use SwiftUI's native `.onContinuousHover` in views that dynamically mount and unmount custom AppKit `NSTextField` editors (like inline editing fields). macOS has a known SwiftUI bug where the field editor's insertion into the responder chain corrupts the `NSTrackingArea`, causing hover events to either freeze completely or enter infinite loops. Instead, use a custom `NSViewRepresentable` that manages its own `NSTrackingArea` internally (e.g., `HoverTrackingView`).
10. **NSViewRepresentable Size Defaulting**: If an `NSViewRepresentable` lacks intrinsic content size constraints and you use it as a `.background()`, you MUST implement `func sizeThatFits(_ proposal: ProposedViewSize, ...)` to return `proposal.replacingUnspecifiedDimensions`. Otherwise, SwiftUI will assign it a `.zero` frame, causing any `frameInWindow.contains(event.location)` logic inside local event monitors to instantly and permanently fail (leading to instantaneous dismissal of inline UI like `ClickOutsideMonitorView`).
