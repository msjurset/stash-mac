...
- **Identify with Gemini** — Right-click any image item → Identify with Gemini to send the photo to Google Gemini and slot the returned title/notes back into the item. Title is filled only when blank; notes are appended (blank-line separated) so repeat identifies don't lose earlier output. Set the API key and edit the prompt in Settings → Gemini; the key never leaves this Mac
