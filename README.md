...
- **Advanced Search** — Search query supports `collection:name` to filter by collection, `tag:name` for tags, and operators: `!tag:name` (not tagged), `-tag:name` (exclude), and `^tag:name` (exact tag match). Global commands like `/today`, `/yesterday`, `/untagged`, and `/favorites` available directly from the search bar.
